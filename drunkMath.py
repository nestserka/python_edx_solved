...
def to_metric(n):
    for token in n.split():
        if "cups" in n:
            test = float(token) * 240
            return int(test)
        elif "ounces" in n:
            test = float(token) * 29.57
            t = str(round(test, 2))
            t = float(t)
            return t
        elif "tablespoons" in n:
            test = float(token) * 14.79
            t = str(round(test, 2))
            t = float(t)
            return t
        elif "teaspoons" in n:
            test = float(token) * 4.93
            t = str(round(test, 2))
            t = float(t)
            return t
        elif "gallons" in n:
            test = float(token) * 3785.41
            t = str(round(test, 2))
            t = float(t)
            return t
        elif "quarts" in n:
            test = float(token) * 946.35
            t = str(round(test, 2))
            t = float(t)
            return t
        else:
            test = float(token) * 473.18
            t = str(round(test, 2))
            t = float(t)
            return t
